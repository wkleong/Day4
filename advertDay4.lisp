...
(defvar rawRoomList "aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
(defvar rawRoomList2 "aaaaa-bbb-z-y-x-123[abxyz] a-b-c-d-e-f-g-h-987[abcde] not-a-real-room-404[oarel] totally-real-room-200[decoy]")
(defun from-string-to-list (sss)
  (let ((L (read-from-string 
           (concatenate 'string "(" sss ")"))))
    L))

(defun ssubString (stringg)
(let (s1 s2 s3)
    (setq s1 (substitute #\space #\- stringg))
    (setq s2 (substitute #\space #\[ s1))
    (setq s3 (substitute #\space #\] s2))
s3
))

(defun genRoomList (rList)
(let (roomList)
  (dolist (roomm rList)
    (if (= (length roomList) 0)
        (setq roomList (list (ssubString (string roomm))))
        (setq roomList (cons (ssubString (string roomm)) roomList))
    )
  )
  (reverse roomList)
))

(defun countStringChar (stringg charr)
(let (lenOrig (lenFinal 0) s2 runList atomChar)
    (setq lenOrig (length stringg))
    (setq runList (loop for i from 0 to (- lenOrig 1) collect i))
    (dolist (i runList)
        (setq atomChar (subseq stringg i (+ i 1)))
        (when (string= atomChar charr) (setq lenFinal (+ lenFinal 1)))
    )
lenFinal
))

(defun countChar (checkS rList)
(let (useList checkSumList checkSumCntList uList uList2 mainList)
    (dolist (ii (loop for i from 0 to (- (length rList) 3) collect i))
        (setq wordd (string (nth ii rList)))
        (setq loopWordList (loop for i from 0 to (- (length wordd) 1) collect i))
        (dolist (iii (loop for i from 0 to (- (length wordd) 1) collect i))
            (setq charr (subseq (string wordd) iii (1+ iii)))
            (setq sC (intern charr))
            (when (not (member sC useList))
                (setq useList (cons sC useList))
                (setq checkSumList (cons charr checkSumList))
            )
            (if (not (get sC sC))
                (setf (get sC sC) 1)
                (setf (get sC sC) (1+ (get sC sC)))
            )
        )
    )
    (setq checkSumList (reverse checkSumList))
    (dolist (sC useList)
        (setq checkSumCntList (cons (get sC sC) checkSumCntList))
    )

    (dolist (itemm (loop for i from 0 to (- (length checkSumList) 1) collect i))
        (setq s1 (nth itemm checkSumCntList))
        (setq ss (intern (write-to-string (nth itemm checkSumCntList))))
        (when (not (member ss uList))
            (setq uList (cons ss uList))
            (setq uList2 (cons s1 uList2))
        )
        (setf (get ss ss) (concatenate 'string (nth itemm checkSumList) (get ss ss)))
        (setf (get ss ss) (sort (get ss ss) #'char-lessp))
    )
    (setq uList2 (sort (copy-list uList2) #'>))
    (setq uList (reverse uList))

    (dolist (uu uList2)
        (setq ss (intern (write-to-string uu)))
        (setq mainList (concatenate 'string mainList (get ss ss)))
    )
    (dolist (uu uList)
        (setf (get uu uu) "")
    )
...
(string= checkS (subseq mainList 0 5))
))

(defun processRoom (roomList)
(let (rList checkS numm sizeS summ)
    (setq summ 0)
    (dolist (romm roomList)
        (setq rList (from-string-to-list romm))
        (setq sizeS (- (length rList) 1))
        (setq checkS (nth sizeS rList))
        (setq numm (nth (- sizeS 1) rList))
        (when (countChar checkS rList)
            (setq summ (+ summ numm))
        )
        ;(format t "~% sum ~D" summ)
    );dolist
...
(defun main (roomList)
    (format t "~%Day 4: The sum of the sector IDs of the real rooms is ~D" (processRoom (genRoomList (from-string-to-list roomList))))
)
